...
it, then run this script. Up to three files are written and never drift apart:
    ...
    ts/packages/identity/src/handleVectors.ts       # TypeScript constants + vectors

The Rust and TypeScript packages do not exist in this repo yet; their outputs
are skipped with a note until their target directories appear. The normalizer
...
# The Rust and TypeScript packages are not in this repo yet. Each output is
# skipped (with a note) while its parent directory is absent, so `--check`
# holds only the Solidity output to account today.
RUST_OUT = REPO_ROOT / "rust" / "identity" / "src" / "handle_vectors.rs"
TS_OUT = REPO_ROOT / "ts" / "packages" / "identity" / "src" / "handleVectors.ts"
...
def main() -> int:
    ...
    # The Rust and TS packages are not in this repo yet: emit their outputs
    # only once someone has created the package directories.
